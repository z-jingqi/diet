...
export const RecipeNutrientSchema = z.object({
  calories: z.number().nonnegative().describe("热量 ‑ kcal"),
  protein: z.number().nonnegative().describe("蛋白质 ‑ g"),
  fat: z.number().nonnegative().describe("脂肪 ‑ g"),
  carbs: z.number().nonnegative().describe("碳水化合物 ‑ g"),
  fiber: z.number().nonnegative().describe("膳食纤维 ‑ g"),
  sodium: z.number().nonnegative().describe("钠 ‑ mg"),
  sugar: z.number().nonnegative().describe("糖 ‑ g"),
});
...
export const RecipeIngredientSchema = z.object({
  ...
  substitutes: z.array(z.string()).optional().describe("可替代食材名称列表"),
...
export const RecipeStepSchema = z.object({
  ...
  durationApproxMin: z
    ...
    .optional()
    .describe("预估耗时（分钟，可选）"),
  imageUrl: z.string().url().optional().describe("步骤配图 URL（预留）"),
});
...
export const RecipeSchema = z.object({
  ...
  coverImageUrl: z.string().url().optional().describe("主图 URL（预留）"),
  ...
  totalTimeApproxMin: z
    ...
    .nonnegative()
    .describe("总耗时（估算，分钟）"),
  costApprox: z.number().nonnegative().describe("参考花费（单份，估算）"),
  currency: z.string().length(3).describe("货币代码，ISO 4217，例如 CNY"),
  ...
  leftoverHandling: z.string().optional().describe("剩菜处理建议"),
  ...
  updatedAt: z.string().describe("最近更新时间，ISO 字符串"),

  // 与聊天消息关联
  sourceMessageId: z.string().describe("生成该菜谱所依据的聊天消息 ID"),

  // 关联字段
  ingredients: z.array(RecipeIngredientSchema).min(1).describe("食材列表"),
