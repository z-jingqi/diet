...
async function seedLocalDatabase() {
  try {
    ...
    for (const clearCommand of clearCommands) {
      try {
        const clearSql = `npx wrangler d1 execute DB --local --command="${clearCommand}"`;
    ...
    const command = `npx wrangler d1 execute DB --local --file=${seedDataPath}`;
    ...
    console.log("✅ 本地数据库更新成功！");
    console.log(result);
  } catch (error) {
    console.error("❌ 本地数据库更新失败:", error);
