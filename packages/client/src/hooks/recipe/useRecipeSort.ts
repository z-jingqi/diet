 ...
 * 菜谱排序hook
 */
export const useRecipeSort = (recipes: BasicRecipeInfo[], sortConfig: SortConfig) => {
  ...
  const sortedRecipes = useMemo(() => {
    ...
    if (sortConfig.difficulty !== "default") {
      ...
      sorted.sort((a, b) => {
        const difficultyA = difficultyOrder[a.difficulty as keyof typeof difficultyOrder] || 2;
        const difficultyB = difficultyOrder[b.difficulty as keyof typeof difficultyOrder] || 2;
    ...
    if (sortConfig.status !== "default") {
      sorted.sort((a, b) => {
        const statusA = getRecipeStatus(a.name, isGenerating, myRecipesData?.myRecipes || []);
        const statusB = getRecipeStatus(b.name, isGenerating, myRecipesData?.myRecipes || []);
