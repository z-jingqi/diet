...
const TagList = memo(
  ({
    ...
    // console.log("Selected Tags IDs:", Array.from(selectedTagIds));
            ...
            <div className="flex flex-wrap gap-2">
              {tagsData?.tags
                ...
                .map((tag) => {
                  ...
                  // Debug this specific tag - remove after fix
                  // console.log(
                  //   `Tag ${tag.name} (${tag.id}): selected=${isSelected}`
                  // );
                    ...
                    <Badge
                      ...
                      onClick={() => {
                        if (isDisabled) {
                          // 显示禁用原因
                          const description =
                            (tag.id && conflictDescriptions[tag.id]) ||
                            "标签冲突，无法同时选择";
